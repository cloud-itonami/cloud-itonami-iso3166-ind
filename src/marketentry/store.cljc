(ns marketentry.store
  "SSoT for the IND market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems IN" :portal "GeM"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-in-entity? true :has-in-entity? true
             :requires-gstin? true :gstin-verified? true
             :drafted? false :submitted? false
             :jurisdiction "IND" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "GeM"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-in-entity? true :has-in-entity? true
             :requires-gstin? true :gstin-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "GeM"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "IND" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "GeM"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-in-entity? true :has-in-entity? false
             :requires-gstin? true :gstin-verified? true
             :drafted? false :submitted? false
             :jurisdiction "IND" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "GeM"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-in-entity? true :has-in-entity? true
             :requires-gstin? true :gstin-verified? false
             :drafted? false :submitted? false
             :jurisdiction "IND" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "GeM"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               requires-in-entity? has-in-entity?
                               requires-gstin? gstin-verified?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-in-entity?)  (assoc :engagement/requires-in-entity? requires-in-entity?)
    (some? has-in-entity?)       (assoc :engagement/has-in-entity? has-in-entity?)
    (some? requires-gstin?)    (assoc :engagement/requires-gstin? requires-gstin?)
    (some? gstin-verified?)    (assoc :engagement/gstin-verified? gstin-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-in-entity? :engagement/has-in-entity?
   :engagement/requires-gstin? :engagement/gstin-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-in-entity? (boolean (:engagement/requires-in-entity? m))
     :has-in-entity? (boolean (:engagement/has-in-entity? m))
     :requires-gstin? (boolean (:engagement/requires-gstin? m))
     :gstin-verified? (boolean (:engagement/gstin-verified? m))
