(ns marketentry.marketentryllm
  ...
  the IND public-sector market-entry compliance actor.
...
(defn- propose-submit
  ...
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "has-in-entity?=" (:has-in-entity? e)
                        " gstin-verified?=" (:gstin-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-in-entity? e))
                              (:has-in-entity? e))
                          (or (not (:requires-gstin? e))
                              (:gstin-verified? e)))
                   0.9 0.3)}))
