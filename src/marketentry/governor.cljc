(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether an India-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. India-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-in-entity?
                                       true` (near-universal for India
                                       ...
                                       Grounded in Indian entity registration
                                       typically required for GeM seller
                                       onboarding (GeM / procuring entities).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(GSTIN/統一資格/GeM登録/代理人確認等)が充足していない状態での提案"}]))))

(defn- in-entity-missing-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-in-entity? true`, INDEPENDENTLY verify
  `:has-in-entity?` is true -- the flagship genuinely new
  check this vertical adds. CONDITIONAL on the engagement's own
  `:requires-in-entity?` ground truth (most India public
  tenders require it; a pure-foreign-market engagement may not)."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-in-entity? e))
                 (not (true? (:has-in-entity? e))))
        [{:rule :in-entity-missing
          :detail (str subject " はインド居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- gstin-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-gstin? true`, INDEPENDENTLY check
  `:gstin-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-gstin? e))
                 (not (true? (:gstin-verified? e))))
        [{:rule :gstin-unverified
          :detail (str subject " はGSTIN確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (in-entity-missing-violations request st)
                           (engagement-fee-mismatch-violations request st)
                           (gstin-unverified-violations request st)
